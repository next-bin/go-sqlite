// Code generated for windows/amd64 by 'gcc --prefix-enumerator=_ --prefix-external=x_ --prefix-field=F --prefix-macro=m_ --prefix-static-internal=_ --prefix-static-none=_ --prefix-tagged-enum=_ --prefix-tagged-struct=T --prefix-tagged-union=T --prefix-typename=T --prefix-undefined=_ -extended-errors --package-name=main minigzip64.o.go -o minigzip64.go', DO NOT EDIT.
...
func x_gz_compress(tls *libc.TLS, in uintptr, out TgzFile) {
	...
	for {
		...
		if _gzwrite(tls, out, bp, uint32(len1)) != len1 {
			x_error(tls, _gzerror(tls, out, bp+16384))
		}
		goto _1
	_1:
	}
	libc.Xfclose(tls, in)
	if _gzclose(tls, out) != m_Z_OK {
...
func x_gz_uncompress(tls *libc.TLS, in TgzFile, out uintptr) {
	...
	_ = len1
	for {
		len1 = _gzread(tls, in, bp, uint32(16384))
		if len1 < 0 {
			x_error(tls, _gzerror(tls, in, bp+16384))
		}
		if len1 == 0 {
			break
		}
...
func x_file_uncompress(tls *libc.TLS, file uintptr) {
	...
	in = _gzopen(tls, infile, __ccgo_ts+89)
	if in == libc.UintptrFromInt32(0) {
...
func x_main(tls *libc.TLS, argc int32, argv uintptr) (r int32) {
	...
	if argc == 0 {
		...
		if uncompr != 0 {
			file = _gzdopen(tls, libc.Xfileno(tls, libc.X__acrt_iob_func(tls, uint32(0))), __ccgo_ts+89)
			if file == libc.UintptrFromInt32(0) {
				x_error(tls, __ccgo_ts+151)
			}
			x_gz_uncompress(tls, file, libc.X__acrt_iob_func(tls, uint32(1)))
		} else {
			file = _gzdopen(tls, libc.Xfileno(tls, libc.X__acrt_iob_func(tls, uint32(1))), bp)
			if file == libc.UintptrFromInt32(0) {
		...
		for {
			if uncompr != 0 {
				if copyout != 0 {
					file = _gzopen(tls, *(*uintptr)(unsafe.Pointer(argv)), __ccgo_ts+89)
					if file == libc.UintptrFromInt32(0) {
