...
static struct Global {
  ...
  int defaultTimeout;    /* Default timeout from command line */
...
static void startClient(int iClient){
  ...
  if( sqlite3_changes(g.db) ){
    ...
    zSys = sqlite3_mprintf("%s \"%s\" --client %d --trace %d --timeout %d",
                 g.argv0, g.zDbFile, iClient, g.iTrace, g.defaultTimeout);   
    if( g.bSqlTrace ){
...
int SQLITE_CDECL main(int argc, char **argv){
  ...
  sqlite3_create_function(g.db, "eval", 1, SQLITE_UTF8, 0,
                          evalFunc, 0, 0);
  g.defaultTimeout = (iTmout > 0) ? iTmout : DEFAULT_TIMEOUT;
  g.iTimeout = g.defaultTimeout;
