						...
						if libc.Xstrcmp(tls, bp+16, __ccgo_ts+1234) == 0 {
							...
							if libc.Xstrcmp(tls, bp+16, __ccgo_ts+1240) == 0 {
								...
								if len1-jj-int32(1) != (*(*String)(unsafe.Pointer(bp + 4))).n || libc.Xstrncmp(tls, (*(*String)(unsafe.Pointer(bp + 4))).z, zAns, uint32(len1-jj-int32(1))) != 0 {
									errorMessage(tls, __ccgo_ts+1246, libc.VaList(bp+1256, prevLine, zFilename, len1-jj-int32(1), zAns, (*(*String)(unsafe.Pointer(bp + 4))).z))
								}
								g.nTest++
								stringReset(tls, bp+4)
							} else {
								...
								if libc.Xstrcmp(tls, bp+16, __ccgo_ts+1291) == 0 || libc.Xstrcmp(tls, bp+16, __ccgo_ts+1296) == 0 {
									...
									if libc.BoolInt32(libsqlite3.Xsqlite3_strglob(tls, zCopy, (*(*String)(unsafe.Pointer(bp + 4))).z) == 0)^isGlob != 0 {
										errorMessage(tls, __ccgo_ts+1304, libc.VaList(bp+1256, prevLine, zFilename, zCopy, (*(*String)(unsafe.Pointer(bp + 4))).z))
									}
									...
									if libc.Xstrcmp(tls, bp+16, __ccgo_ts+1347) == 0 {
										logMessage(tls, __ccgo_ts+438, libc.VaList(bp+1256, (*(*String)(unsafe.Pointer(bp + 4))).z))
									} else {
										/*
										 **  --source FILENAME
										 **
...
func main1(tls *libc.TLS, argc int32, argv uintptr) (r int32) {
	...
	*(*int32)(unsafe.Pointer(bp)) = argc - int32(2)
	...
	g.zVfs = findOption(tls, argv+uintptr(2)*4, bp, __ccgo_ts+2506, int32(1))
	zClient = findOption(tls, argv+uintptr(2)*4, bp, __ccgo_ts+2510, int32(1))
	g.zErrLog = findOption(tls, argv+uintptr(2)*4, bp, __ccgo_ts+2517, int32(1))
	g.zLog = findOption(tls, argv+uintptr(2)*4, bp, __ccgo_ts+2524, int32(1))
	...
	g.bSqlTrace = libc.BoolInt32(findOption(tls, argv+uintptr(2)*4, bp, __ccgo_ts+2548, 0) != uintptr(0))
	g.bSync = libc.BoolInt32(findOption(tls, argv+uintptr(2)*4, bp, __ccgo_ts+2557, 0) != uintptr(0))
	if g.zErrLog != 0 {
	...
	if zClient != 0 {
		...
		for {
			if nTry%int32(5) == int32(4) {
				...
				libc.Xprintf(tls, __ccgo_ts+2742, libc.VaList(bp+24, v7, g.zDbFile))
			}
		...
		if rc != 0 {
			fatalError(tls, __ccgo_ts+2771, libc.VaList(bp+24, g.zDbFile, nTry))
		}
	...
	if rc != 0 {
		fatalError(tls, __ccgo_ts+2812, libc.VaList(bp+24, g.zDbFile))
	}
	if iTmout > 0 {
	...
	maybeClose(tls, g.pErrLog)
	if iClient == 0 {
		libc.Xprintf(tls, __ccgo_ts+3558, libc.VaList(bp+24, g.nError, g.nTest))
