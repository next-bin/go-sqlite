...
func _error(tls *libc.TLS, msg uintptr) {
	bp := tls.Alloc(32)
	defer tls.Free(32)
	libc.Xfprintf(tls, libc.Xstderr, __ccgo_ts, libc.VaList(bp+8, _prog, msg))
...
func _file_compress(tls *libc.TLS, file uintptr, mode uintptr) {
	...
	if libc.Xstrlen(tls, file)+libc.Xstrlen(tls, __ccgo_ts+57) >= uint64(1025) {
		libc.Xfprintf(tls, libc.Xstderr, __ccgo_ts+61, libc.VaList(bp+1040, _prog))
		libc.Xexit(tls, int32(1))
	}
	...
	if out == libc.UintptrFromInt32(0) {
		libc.Xfclose(tls, in)
		libc.Xfprintf(tls, libc.Xstderr, __ccgo_ts+87, libc.VaList(bp+1040, _prog, bp))
		libc.Xexit(tls, int32(1))
	}
...
func _file_uncompress(tls *libc.TLS, file uintptr) {
	...
	if in == libc.UintptrFromInt32(0) {
		libc.Xfprintf(tls, libc.Xstderr, __ccgo_ts+87, libc.VaList(bp+1040, _prog, infile))
		libc.Xexit(tls, int32(1))
	}
...
func x_main(tls *libc.TLS, argc int32, argv uintptr) (r int32) {
	...
	if argc == 0 {
		if uncompr != 0 {
			file = x_gzdopen(tls, libc.Xfileno(tls, libc.Xstdin), __ccgo_ts+84)
			if file == libc.UintptrFromInt32(0) {
				_error(tls, __ccgo_ts+143)
			}
			_gz_uncompress(tls, file, libc.Xstdout)
		} else {
			file = x_gzdopen(tls, libc.Xfileno(tls, libc.Xstdout), bp)
			if file == libc.UintptrFromInt32(0) {
				_error(tls, __ccgo_ts+163)
			}
			_gz_compress(tls, libc.Xstdin, file)
		}
	} else {
		if copyout != 0 {
		}
		for {
			if uncompr != 0 {
				if copyout != 0 {
					...
					if file == libc.UintptrFromInt32(0) {
						libc.Xfprintf(tls, libc.Xstderr, __ccgo_ts+87, libc.VaList(bp+16, _prog, *(*uintptr)(unsafe.Pointer(argv))))
					} else {
						_gz_uncompress(tls, file, libc.Xstdout)
					}
				} else {
