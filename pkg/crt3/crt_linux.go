...
package crt // import "github.com/next-bin/go-sqlite3/pkg/crt3"
...
import (
	...
	ftsh "github.com/next-bin/go-sqlite3/pkg/crt3/libc/fts"
	"github.com/next-bin/go-sqlite3/pkg/crt3/libc/grp"
	"github.com/next-bin/go-sqlite3/pkg/crt3/libc/netdb"
	"github.com/next-bin/go-sqlite3/pkg/crt3/libc/pwd"
	"github.com/next-bin/go-sqlite3/pkg/crt3/libc/stdio"
	"github.com/next-bin/go-sqlite3/pkg/crt3/libc/sys/mman"
	"github.com/next-bin/go-sqlite3/pkg/crt3/libc/unistd"
...
func Xwrite(t *TLS, fd int32, buf uintptr, count Size_t) Ssize_t {
	...
	if err != nil {
		t.setErrno(err)
		// if dmesgs {
		// 	dmesg("%v: write(%d, %#x, %#x): %v", origin(2), fd, buf, count, err)
		// }
		return -1
	}

	if dmesgs {
		dmesg("%v: write(%d, %#x, %#x): %v\n%s", origin(2), fd, buf, count, n, hex.Dump(GoBytes(buf, int(count))))
	}
	return Intptr(n)
}
