%{

//TODO Put your favorite license here
		
// yacc source generated by ebnf2y[1]
// at 2014-10-29 16:19:52.699905041 +0100 CET
//
//  $ ebnf2y go.ebnf
//
// CAUTION: If this file is a Go source file (*.go), it was generated
// automatically by '$ go tool yacc' from a *.y file - DO NOT EDIT in that case!
// 
//   [1]: http://github.com/next-bin/go-sqlite3/pkg/ebnf2y

package main //TODO real package name

//TODO required only be the demo _dump function
import (
	"bytes"
	"fmt"
	"strings"

	"github.com/next-bin/go-sqlite3/pkg/strutil"
)

%}

%union {
	item interface{} //TODO insert real field(s)
}

%token	ASSIGN_OP
%token	BINARY_OP
%token	FLOAT_LIT
%token	IDENTIFIER
%token	IMAGINARY_LIT
%token	INT_LIT
%token	RUNE_LIT
%token	STRING_LIT
%token	UNARY_OP

%type	<item> 	/*TODO real type(s), if/where applicable */
	ASSIGN_OP
	BINARY_OP
	FLOAT_LIT
	IDENTIFIER
	IMAGINARY_LIT
	INT_LIT
	RUNE_LIT
	STRING_LIT
	UNARY_OP

%token	TOK1	/*TODO Name for "--" */
%token	TOK2	/*TODO Name for "++" */
%token	TOK3	/*TODO Name for ":=" */
%token	TOK4	/*TODO Name for "..." */
%token	TOK5	/*TODO Name for "<-" */

%type	<item> 	/*TODO real type(s), if/where applicable */
	TOK1
	TOK2
	TOK3
	TOK4
	TOK5

%token BREAK
%token CASE
%token CHAN
%token CONST
%token CONTINUE
%token DEFAULT
%token DEFER
%token ELSE
%token FALLTHROUGH
%token FOR
%token FUNC
%token GO
%token GOTO
%token IF
%token IMPORT
%token INTERFACE
%token MAP
%token PACKAGE
%token RANGE
%token RETURN
%token SELECT
%token STRUCT
%token SWITCH
%token TYPE
%token VAR

%type	<item> 	/*TODO real type(s), if/where applicable */
	AnonymousField
	AnonymousField1
	ArgumentList
	ArgumentList1
	ArrayLength
	ArrayType
	Assignment
	BaseType
	BaseTypeName
	BasicLit
	Block
	BreakStmt
	BreakStmt1
	BuiltinArgs
	BuiltinArgs1
	BuiltinCall
	BuiltinCall1
	BuiltinCall11
	Call
	Call1
	Call11
	Channel
	ChannelType
	ChannelType1
	CommCase
	CommCase1
	CommClause
	CompositeLit
	Condition
	ConstDecl
	ConstDecl1
	ConstDecl11
	ConstSpec
	ConstSpec1
	ConstSpec11
	ContinueStmt
	ContinueStmt1
	Conversion
	Conversion1
	Declaration
	DeferStmt
	Element
	Element1
	ElementIndex
	ElementList
	ElementList1
	ElementType
	EmptyStmt
	ExprCaseClause
	ExprSwitchCase
	ExprSwitchStmt
	ExprSwitchStmt1
	ExprSwitchStmt2
	ExprSwitchStmt3
	Expression
	ExpressionList
	ExpressionList1
	ExpressionStmt
	FallthroughStmt
	FieldDecl
	FieldDecl1
	FieldDecl2
	FieldName
	ForClause
	ForClause1
	ForClause2
	ForClause3
	ForStmt
	ForStmt1
	ForStmt11
	Function
	FunctionBody
	FunctionDecl
	FunctionDecl1
	FunctionLit
	FunctionName
	FunctionType
	GoStmt
	GotoStmt
	IdentifierList
	IdentifierList1
	IfStmt
	IfStmt1
	IfStmt2
	IfStmt21
	ImportDecl
	ImportDecl1
	ImportDecl11
	ImportPath
	ImportSpec
	ImportSpec1
	ImportSpec11
	IncDecStmt
	IncDecStmt1
	Index
	InitStmt
	InterfaceType
	InterfaceType1
	InterfaceTypeName
	Key
	KeyType
	Label
	LabeledStmt
	Literal
	LiteralType
	LiteralValue
	LiteralValue1
	LiteralValue11
	MapType
	MethodDecl
	MethodDecl1
	MethodExpr
	MethodName
	MethodSpec
	Operand
	OperandName
	PackageClause
	PackageName
	ParameterDecl
	ParameterDecl1
	ParameterDecl2
	ParameterList
	ParameterList1
	Parameters
	Parameters1
	Parameters11
	PointerType
	PostStmt
	PrimaryExpr
	QualifiedIdent
	RangeClause
	RangeClause1
	Receiver
	Receiver1
	Receiver2
	ReceiverType
	RecvExpr
	RecvStmt
	RecvStmt1
	RecvStmt11
	Result
	ReturnStmt
	ReturnStmt1
	SelectStmt
	SelectStmt1
	Selector
	SendStmt
	ShortVarDecl
	Signature
	Signature1
	SimpleStmt
	Slice
	Slice1
	Slice11
	Slice12
	Slice2
	Slice21
	SliceType
	SourceFile
	SourceFile1
	SourceFile2
	Start
	Statement
	StatementList
	StatementList1
	StructType
	StructType1
	SwitchStmt
	Tag
	TopLevelDecl
	Type
	TypeAssertion
	TypeCaseClause
	TypeDecl
	TypeDecl1
	TypeDecl11
	TypeList
	TypeList1
	TypeLit
	TypeName
	TypeSpec
	TypeSwitchCase
	TypeSwitchGuard
	TypeSwitchGuard1
	TypeSwitchStmt
	TypeSwitchStmt1
	TypeSwitchStmt2
	UnaryExpr
	Value
	VarDecl
	VarDecl1
	VarDecl11
	VarSpec
	VarSpec1
	VarSpec11

/*TODO %left, %right, ... declarations */

%start Start

%%

AnonymousField:
	AnonymousField1 TypeName
	{
		$$ = []AnonymousField{$1, $2} //TODO 1
	}

AnonymousField1:
	/* EMPTY */
	{
		$$ = nil //TODO 2
	}
|	'*'
	{
		$$ = "*" //TODO 3
	}

ArgumentList:
	ExpressionList ArgumentList1
	{
		$$ = []ArgumentList{$1, $2} //TODO 4
	}

ArgumentList1:
	/* EMPTY */
	{
		$$ = nil //TODO 5
	}
|	TOK4 /* "..." */
	{
		$$ = "..." //TODO 6
	}

ArrayLength:
	Expression
	{
		$$ = $1 //TODO 7
	}

ArrayType:
	'[' ArrayLength ']' ElementType
	{
		$$ = []ArrayType{"[", $2, "]", $4} //TODO 8
	}

Assignment:
	ExpressionList ASSIGN_OP ExpressionList
	{
		$$ = []Assignment{$1, $2, $3} //TODO 9
	}

BaseType:
	Type
	{
		$$ = $1 //TODO 10
	}

BaseTypeName:
	IDENTIFIER
	{
		$$ = $1 //TODO 11
	}

BasicLit:
	INT_LIT
	{
		$$ = $1 //TODO 12
	}
|	FLOAT_LIT
	{
		$$ = $1 //TODO 13
	}
|	IMAGINARY_LIT
	{
		$$ = $1 //TODO 14
	}
|	RUNE_LIT
	{
		$$ = $1 //TODO 15
	}
|	STRING_LIT
	{
		$$ = $1 //TODO 16
	}

Block:
	'{' StatementList '}'
	{
		$$ = []Block{"{", $2, "}"} //TODO 17
	}

BreakStmt:
	BREAK BreakStmt1
	{
		$$ = []BreakStmt{"break", $2} //TODO 18
	}

BreakStmt1:
	/* EMPTY */
	{
		$$ = nil //TODO 19
	}
|	Label
	{
		$$ = $1 //TODO 20
	}

BuiltinArgs:
	Type BuiltinArgs1
	{
		$$ = []BuiltinArgs{$1, $2} //TODO 21
	}
|	ArgumentList
	{
		$$ = $1 //TODO 22
	}

BuiltinArgs1:
	/* EMPTY */
	{
		$$ = nil //TODO 23
	}
|	',' ArgumentList
	{
		$$ = []BuiltinArgs1{",", $2} //TODO 24
	}

BuiltinCall:
	IDENTIFIER '(' BuiltinCall1 ')'
	{
		$$ = []BuiltinCall{$1, "(", $3, ")"} //TODO 25
	}

BuiltinCall1:
	/* EMPTY */
	{
		$$ = nil //TODO 26
	}
|	BuiltinArgs BuiltinCall11
	{
		$$ = []BuiltinCall1{$1, $2} //TODO 27
	}

BuiltinCall11:
	/* EMPTY */
	{
		$$ = nil //TODO 28
	}
|	','
	{
		$$ = "," //TODO 29
	}

Call:
	'(' Call1 ')'
	{
		$$ = []Call{"(", $2, ")"} //TODO 30
	}

Call1:
	/* EMPTY */
	{
		$$ = nil //TODO 31
	}
|	ArgumentList Call11
	{
		$$ = []Call1{$1, $2} //TODO 32
	}

Call11:
	/* EMPTY */
	{
		$$ = nil //TODO 33
	}
|	','
	{
		$$ = "," //TODO 34
	}

Channel:
	Expression
	{
		$$ = $1 //TODO 35
	}

ChannelType:
	ChannelType1 ElementType
	{
		$$ = []ChannelType{$1, $2} //TODO 36
	}

ChannelType1:
	CHAN
	{
		$$ = "chan" //TODO 37
	}
|	CHAN TOK5 /* "<-" */
	{
		$$ = []ChannelType1{"chan", "<-"} //TODO 38
	}
|	TOK5 /* "<-" */ CHAN
	{
		$$ = []ChannelType1{"<-", "chan"} //TODO 39
	}

CommCase:
	CASE CommCase1
	{
		$$ = []CommCase{"case", $2} //TODO 40
	}
|	DEFAULT
	{
		$$ = "default" //TODO 41
	}

CommCase1:
	SendStmt
	{
		$$ = $1 //TODO 42
	}
|	RecvStmt
	{
		$$ = $1 //TODO 43
	}

CommClause:
	CommCase ':' StatementList
	{
		$$ = []CommClause{$1, ":", $3} //TODO 44
	}

CompositeLit:
	LiteralType LiteralValue
	{
		$$ = []CompositeLit{$1, $2} //TODO 45
	}

Condition:
	Expression
	{
		$$ = $1 //TODO 46
	}

ConstDecl:
	CONST ConstDecl1
	{
		$$ = []ConstDecl{"const", $2} //TODO 47
	}

ConstDecl1:
	ConstSpec
	{
		$$ = $1 //TODO 48
	}
|	'(' ConstDecl11 ')'
	{
		$$ = []ConstDecl1{"(", $2, ")"} //TODO 49
	}

ConstDecl11:
	/* EMPTY */
	{
		$$ = []ConstDecl11(nil) //TODO 50
	}
|	ConstDecl11 ConstSpec ';'
	{
		$$ = append($1.([]ConstDecl11), $2, ";") //TODO 51
	}

ConstSpec:
	IdentifierList ConstSpec1
	{
		$$ = []ConstSpec{$1, $2} //TODO 52
	}

ConstSpec1:
	/* EMPTY */
	{
		$$ = nil //TODO 53
	}
|	ConstSpec11 '=' ExpressionList
	{
		$$ = []ConstSpec1{$1, "=", $3} //TODO 54
	}

ConstSpec11:
	/* EMPTY */
	{
		$$ = nil //TODO 55
	}
|	Type
	{
		$$ = $1 //TODO 56
	}

ContinueStmt:
	CONTINUE ContinueStmt1
	{
		$$ = []ContinueStmt{"continue", $2} //TODO 57
	}

ContinueStmt1:
	/* EMPTY */
	{
		$$ = nil //TODO 58
	}
|	Label
	{
		$$ = $1 //TODO 59
	}

Conversion:
	Type '(' Expression Conversion1 ')'
	{
		$$ = []Conversion{$1, "(", $3, $4, ")"} //TODO 60
	}

Conversion1:
	/* EMPTY */
	{
		$$ = nil //TODO 61
	}
|	','
	{
		$$ = "," //TODO 62
	}

Declaration:
	ConstDecl
	{
		$$ = $1 //TODO 63
	}
|	TypeDecl
	{
		$$ = $1 //TODO 64
	}
|	VarDecl
	{
		$$ = $1 //TODO 65
	}

DeferStmt:
	DEFER Expression
	{
		$$ = []DeferStmt{"defer", $2} //TODO 66
	}

Element:
	Element1 Value
	{
		$$ = []Element{$1, $2} //TODO 67
	}

Element1:
	/* EMPTY */
	{
		$$ = nil //TODO 68
	}
|	Key ':'
	{
		$$ = []Element1{$1, ":"} //TODO 69
	}

ElementIndex:
	Expression
	{
		$$ = $1 //TODO 70
	}

ElementList:
	Element ElementList1
	{
		$$ = []ElementList{$1, $2} //TODO 71
	}

ElementList1:
	/* EMPTY */
	{
		$$ = []ElementList1(nil) //TODO 72
	}
|	ElementList1 ',' Element
	{
		$$ = append($1.([]ElementList1), ",", $3) //TODO 73
	}

ElementType:
	Type
	{
		$$ = $1 //TODO 74
	}

EmptyStmt:
	/* EMPTY */
	{
		$$ = nil //TODO 75
	}

ExprCaseClause:
	ExprSwitchCase ':' StatementList
	{
		$$ = []ExprCaseClause{$1, ":", $3} //TODO 76
	}

ExprSwitchCase:
	CASE ExpressionList
	{
		$$ = []ExprSwitchCase{"case", $2} //TODO 77
	}
|	DEFAULT
	{
		$$ = "default" //TODO 78
	}

ExprSwitchStmt:
	SWITCH ExprSwitchStmt1 ExprSwitchStmt2 '{' ExprSwitchStmt3 '}'
	{
		$$ = []ExprSwitchStmt{"switch", $2, $3, "{", $5, "}"} //TODO 79
	}

ExprSwitchStmt1:
	/* EMPTY */
	{
		$$ = nil //TODO 80
	}
|	SimpleStmt ';'
	{
		$$ = []ExprSwitchStmt1{$1, ";"} //TODO 81
	}

ExprSwitchStmt2:
	/* EMPTY */
	{
		$$ = nil //TODO 82
	}
|	Expression
	{
		$$ = $1 //TODO 83
	}

ExprSwitchStmt3:
	/* EMPTY */
	{
		$$ = []ExprSwitchStmt3(nil) //TODO 84
	}
|	ExprSwitchStmt3 ExprCaseClause
	{
		$$ = append($1.([]ExprSwitchStmt3), $2) //TODO 85
	}

Expression:
	UnaryExpr
	{
		$$ = $1 //TODO 86
	}
|	Expression BINARY_OP UnaryExpr
	{
		$$ = []Expression{$1, $2, $3} //TODO 87
	}

ExpressionList:
	Expression ExpressionList1
	{
		$$ = []ExpressionList{$1, $2} //TODO 88
	}

ExpressionList1:
	/* EMPTY */
	{
		$$ = []ExpressionList1(nil) //TODO 89
	}
|	ExpressionList1 ',' Expression
	{
		$$ = append($1.([]ExpressionList1), ",", $3) //TODO 90
	}

ExpressionStmt:
	Expression
	{
		$$ = $1 //TODO 91
	}

FallthroughStmt:
	FALLTHROUGH
	{
		$$ = "fallthrough" //TODO 92
	}

FieldDecl:
	FieldDecl1 FieldDecl2
	{
		$$ = []FieldDecl{$1, $2} //TODO 93
	}

FieldDecl1:
	IdentifierList Type
	{
		$$ = []FieldDecl1{$1, $2} //TODO 94
	}
|	AnonymousField
	{
		$$ = $1 //TODO 95
	}

FieldDecl2:
	/* EMPTY */
	{
		$$ = nil //TODO 96
	}
|	Tag
	{
		$$ = $1 //TODO 97
	}

FieldName:
	IDENTIFIER
	{
		$$ = $1 //TODO 98
	}

ForClause:
	ForClause1 ';' ForClause2 ';' ForClause3
	{
		$$ = []ForClause{$1, ";", $3, ";", $5} //TODO 99
	}

ForClause1:
	/* EMPTY */
	{
		$$ = nil //TODO 100
	}
|	InitStmt
	{
		$$ = $1 //TODO 101
	}

ForClause2:
	/* EMPTY */
	{
		$$ = nil //TODO 102
	}
|	Condition
	{
		$$ = $1 //TODO 103
	}

ForClause3:
	/* EMPTY */
	{
		$$ = nil //TODO 104
	}
|	PostStmt
	{
		$$ = $1 //TODO 105
	}

ForStmt:
	FOR ForStmt1 Block
	{
		$$ = []ForStmt{"for", $2, $3} //TODO 106
	}

ForStmt1:
	/* EMPTY */
	{
		$$ = nil //TODO 107
	}
|	ForStmt11
	{
		$$ = $1 //TODO 108
	}

ForStmt11:
	Condition
	{
		$$ = $1 //TODO 109
	}
|	ForClause
	{
		$$ = $1 //TODO 110
	}
|	RangeClause
	{
		$$ = $1 //TODO 111
	}

Function:
	Signature FunctionBody
	{
		$$ = []Function{$1, $2} //TODO 112
	}

FunctionBody:
	Block
	{
		$$ = $1 //TODO 113
	}

FunctionDecl:
	FUNC FunctionName FunctionDecl1
	{
		$$ = []FunctionDecl{"func", $2, $3} //TODO 114
	}

FunctionDecl1:
	Function
	{
		$$ = $1 //TODO 115
	}
|	Signature
	{
		$$ = $1 //TODO 116
	}

FunctionLit:
	FUNC Function
	{
		$$ = []FunctionLit{"func", $2} //TODO 117
	}

FunctionName:
	IDENTIFIER
	{
		$$ = $1 //TODO 118
	}

FunctionType:
	FUNC Signature
	{
		$$ = []FunctionType{"func", $2} //TODO 119
	}

GoStmt:
	GO Expression
	{
		$$ = []GoStmt{"go", $2} //TODO 120
	}

GotoStmt:
	GOTO Label
	{
		$$ = []GotoStmt{"goto", $2} //TODO 121
	}

IdentifierList:
	IDENTIFIER IdentifierList1
	{
		$$ = []IdentifierList{$1, $2} //TODO 122
	}

IdentifierList1:
	/* EMPTY */
	{
		$$ = []IdentifierList1(nil) //TODO 123
	}
|	IdentifierList1 ',' IDENTIFIER
	{
		$$ = append($1.([]IdentifierList1), ",", $3) //TODO 124
	}

IfStmt:
	IF IfStmt1 Expression Block IfStmt2
	{
		$$ = []IfStmt{"if", $2, $3, $4, $5} //TODO 125
	}

IfStmt1:
	/* EMPTY */
	{
		$$ = nil //TODO 126
	}
|	SimpleStmt ';'
	{
		$$ = []IfStmt1{$1, ";"} //TODO 127
	}

IfStmt2:
	/* EMPTY */
	{
		$$ = nil //TODO 128
	}
|	ELSE IfStmt21
	{
		$$ = []IfStmt2{"else", $2} //TODO 129
	}

IfStmt21:
	IfStmt
	{
		$$ = $1 //TODO 130
	}
|	Block
	{
		$$ = $1 //TODO 131
	}

ImportDecl:
	IMPORT ImportDecl1
	{
		$$ = []ImportDecl{"import", $2} //TODO 132
	}

ImportDecl1:
	ImportSpec
	{
		$$ = $1 //TODO 133
	}
|	'(' ImportDecl11 ')'
	{
		$$ = []ImportDecl1{"(", $2, ")"} //TODO 134
	}

ImportDecl11:
	/* EMPTY */
	{
		$$ = []ImportDecl11(nil) //TODO 135
	}
|	ImportDecl11 ImportSpec ';'
	{
		$$ = append($1.([]ImportDecl11), $2, ";") //TODO 136
	}

ImportPath:
	STRING_LIT
	{
		$$ = $1 //TODO 137
	}

ImportSpec:
	ImportSpec1 ImportPath
	{
		$$ = []ImportSpec{$1, $2} //TODO 138
	}

ImportSpec1:
	/* EMPTY */
	{
		$$ = nil //TODO 139
	}
|	ImportSpec11
	{
		$$ = $1 //TODO 140
	}

ImportSpec11:
	'.'
	{
		$$ = "." //TODO 141
	}
|	PackageName
	{
		$$ = $1 //TODO 142
	}

IncDecStmt:
	Expression IncDecStmt1
	{
		$$ = []IncDecStmt{$1, $2} //TODO 143
	}

IncDecStmt1:
	TOK2 /* "++" */
	{
		$$ = "++" //TODO 144
	}
|	TOK1 /* "--" */
	{
		$$ = "--" //TODO 145
	}

Index:
	'[' Expression ']'
	{
		$$ = []Index{"[", $2, "]"} //TODO 146
	}

InitStmt:
	SimpleStmt
	{
		$$ = $1 //TODO 147
	}

InterfaceType:
	INTERFACE '{' InterfaceType1 '}'
	{
		$$ = []InterfaceType{"interface", "{", $3, "}"} //TODO 148
	}

InterfaceType1:
	/* EMPTY */
	{
		$$ = []InterfaceType1(nil) //TODO 149
	}
|	InterfaceType1 MethodSpec ';'
	{
		$$ = append($1.([]InterfaceType1), $2, ";") //TODO 150
	}

InterfaceTypeName:
	TypeName
	{
		$$ = $1 //TODO 151
	}

Key:
	FieldName
	{
		$$ = $1 //TODO 152
	}
|	ElementIndex
	{
		$$ = $1 //TODO 153
	}

KeyType:
	Type
	{
		$$ = $1 //TODO 154
	}

Label:
	IDENTIFIER
	{
		$$ = $1 //TODO 155
	}

LabeledStmt:
	Label ':' Statement
	{
		$$ = []LabeledStmt{$1, ":", $3} //TODO 156
	}

Literal:
	BasicLit
	{
		$$ = $1 //TODO 157
	}
|	CompositeLit
	{
		$$ = $1 //TODO 158
	}
|	FunctionLit
	{
		$$ = $1 //TODO 159
	}

LiteralType:
	StructType
	{
		$$ = $1 //TODO 160
	}
|	ArrayType
	{
		$$ = $1 //TODO 161
	}
|	'[' TOK4 /* "..." */ ']' ElementType
	{
		$$ = []LiteralType{"[", "...", "]", $4} //TODO 162
	}
|	SliceType
	{
		$$ = $1 //TODO 163
	}
|	MapType
	{
		$$ = $1 //TODO 164
	}
|	TypeName
	{
		$$ = $1 //TODO 165
	}

LiteralValue:
	'{' LiteralValue1 '}'
	{
		$$ = []LiteralValue{"{", $2, "}"} //TODO 166
	}

LiteralValue1:
	/* EMPTY */
	{
		$$ = nil //TODO 167
	}
|	ElementList LiteralValue11
	{
		$$ = []LiteralValue1{$1, $2} //TODO 168
	}

LiteralValue11:
	/* EMPTY */
	{
		$$ = nil //TODO 169
	}
|	','
	{
		$$ = "," //TODO 170
	}

MapType:
	MAP '[' KeyType ']' ElementType
	{
		$$ = []MapType{"map", "[", $3, "]", $5} //TODO 171
	}

MethodDecl:
	FUNC Receiver MethodName MethodDecl1
	{
		$$ = []MethodDecl{"func", $2, $3, $4} //TODO 172
	}

MethodDecl1:
	Function
	{
		$$ = $1 //TODO 173
	}
|	Signature
	{
		$$ = $1 //TODO 174
	}

MethodExpr:
	ReceiverType '.' MethodName
	{
		$$ = []MethodExpr{$1, ".", $3} //TODO 175
	}

MethodName:
	IDENTIFIER
	{
		$$ = $1 //TODO 176
	}

MethodSpec:
	MethodName Signature
	{
		$$ = []MethodSpec{$1, $2} //TODO 177
	}
|	InterfaceTypeName
	{
		$$ = $1 //TODO 178
	}

Operand:
	Literal
	{
		$$ = $1 //TODO 179
	}
|	OperandName
	{
		$$ = $1 //TODO 180
	}
|	MethodExpr
	{
		$$ = $1 //TODO 181
	}
|	'(' Expression ')'
	{
		$$ = []Operand{"(", $2, ")"} //TODO 182
	}

OperandName:
	IDENTIFIER
	{
		$$ = $1 //TODO 183
	}
|	QualifiedIdent
	{
		$$ = $1 //TODO 184
	}

PackageClause:
	PACKAGE PackageName
	{
		$$ = []PackageClause{"package", $2} //TODO 185
	}

PackageName:
	IDENTIFIER
	{
		$$ = $1 //TODO 186
	}

ParameterDecl:
	ParameterDecl1 ParameterDecl2 Type
	{
		$$ = []ParameterDecl{$1, $2, $3} //TODO 187
	}

ParameterDecl1:
	/* EMPTY */
	{
		$$ = nil //TODO 188
	}
|	IdentifierList
	{
		$$ = $1 //TODO 189
	}

ParameterDecl2:
	/* EMPTY */
	{
		$$ = nil //TODO 190
	}
|	TOK4 /* "..." */
	{
		$$ = "..." //TODO 191
	}

ParameterList:
	ParameterDecl ParameterList1
	{
		$$ = []ParameterList{$1, $2} //TODO 192
	}

ParameterList1:
	/* EMPTY */
	{
		$$ = []ParameterList1(nil) //TODO 193
	}
|	ParameterList1 ',' ParameterDecl
	{
		$$ = append($1.([]ParameterList1), ",", $3) //TODO 194
	}

Parameters:
	'(' Parameters1 ')'
	{
		$$ = []Parameters{"(", $2, ")"} //TODO 195
	}

Parameters1:
	/* EMPTY */
	{
		$$ = nil //TODO 196
	}
|	ParameterList Parameters11
	{
		$$ = []Parameters1{$1, $2} //TODO 197
	}

Parameters11:
	/* EMPTY */
	{
		$$ = nil //TODO 198
	}
|	','
	{
		$$ = "," //TODO 199
	}

PointerType:
	'*' BaseType
	{
		$$ = []PointerType{"*", $2} //TODO 200
	}

PostStmt:
	SimpleStmt
	{
		$$ = $1 //TODO 201
	}

PrimaryExpr:
	Operand
	{
		$$ = $1 //TODO 202
	}
|	Conversion
	{
		$$ = $1 //TODO 203
	}
|	BuiltinCall
	{
		$$ = $1 //TODO 204
	}
|	PrimaryExpr Selector
	{
		$$ = []PrimaryExpr{$1, $2} //TODO 205
	}
|	PrimaryExpr Index
	{
		$$ = []PrimaryExpr{$1, $2} //TODO 206
	}
|	PrimaryExpr Slice
	{
		$$ = []PrimaryExpr{$1, $2} //TODO 207
	}
|	PrimaryExpr TypeAssertion
	{
		$$ = []PrimaryExpr{$1, $2} //TODO 208
	}
|	PrimaryExpr Call
	{
		$$ = []PrimaryExpr{$1, $2} //TODO 209
	}

QualifiedIdent:
	PackageName '.' IDENTIFIER
	{
		$$ = []QualifiedIdent{$1, ".", $3} //TODO 210
	}

RangeClause:
	RangeClause1 RANGE Expression
	{
		$$ = []RangeClause{$1, "range", $3} //TODO 211
	}

RangeClause1:
	ExpressionList '='
	{
		$$ = []RangeClause1{$1, "="} //TODO 212
	}
|	IdentifierList TOK3 /* ":=" */
	{
		$$ = []RangeClause1{$1, ":="} //TODO 213
	}

Receiver:
	'(' Receiver1 Receiver2 BaseTypeName ')'
	{
		$$ = []Receiver{"(", $2, $3, $4, ")"} //TODO 214
	}

Receiver1:
	/* EMPTY */
	{
		$$ = nil //TODO 215
	}
|	IDENTIFIER
	{
		$$ = $1 //TODO 216
	}

Receiver2:
	/* EMPTY */
	{
		$$ = nil //TODO 217
	}
|	'*'
	{
		$$ = "*" //TODO 218
	}

ReceiverType:
	TypeName
	{
		$$ = $1 //TODO 219
	}
|	'(' '*' TypeName ')'
	{
		$$ = []ReceiverType{"(", "*", $3, ")"} //TODO 220
	}
|	'(' ReceiverType ')'
	{
		$$ = []ReceiverType{"(", $2, ")"} //TODO 221
	}

RecvExpr:
	Expression
	{
		$$ = $1 //TODO 222
	}

RecvStmt:
	RecvStmt1 RecvExpr
	{
		$$ = []RecvStmt{$1, $2} //TODO 223
	}

RecvStmt1:
	/* EMPTY */
	{
		$$ = nil //TODO 224
	}
|	RecvStmt11
	{
		$$ = $1 //TODO 225
	}

RecvStmt11:
	ExpressionList '='
	{
		$$ = []RecvStmt11{$1, "="} //TODO 226
	}
|	IdentifierList TOK3 /* ":=" */
	{
		$$ = []RecvStmt11{$1, ":="} //TODO 227
	}

Result:
	Parameters
	{
		$$ = $1 //TODO 228
	}
|	Type
	{
		$$ = $1 //TODO 229
	}

ReturnStmt:
	RETURN ReturnStmt1
	{
		$$ = []ReturnStmt{"return", $2} //TODO 230
	}

ReturnStmt1:
	/* EMPTY */
	{
		$$ = nil //TODO 231
	}
|	ExpressionList
	{
		$$ = $1 //TODO 232
	}

SelectStmt:
	SELECT '{' SelectStmt1 '}'
	{
		$$ = []SelectStmt{"select", "{", $3, "}"} //TODO 233
	}

SelectStmt1:
	/* EMPTY */
	{
		$$ = []SelectStmt1(nil) //TODO 234
	}
|	SelectStmt1 CommClause
	{
		$$ = append($1.([]SelectStmt1), $2) //TODO 235
	}

Selector:
	'.' IDENTIFIER
	{
		$$ = []Selector{".", $2} //TODO 236
	}

SendStmt:
	Channel TOK5 /* "<-" */ Expression
	{
		$$ = []SendStmt{$1, "<-", $3} //TODO 237
	}

ShortVarDecl:
	IdentifierList TOK3 /* ":=" */ ExpressionList
	{
		$$ = []ShortVarDecl{$1, ":=", $3} //TODO 238
	}

Signature:
	Parameters Signature1
	{
		$$ = []Signature{$1, $2} //TODO 239
	}

Signature1:
	/* EMPTY */
	{
		$$ = nil //TODO 240
	}
|	Result
	{
		$$ = $1 //TODO 241
	}

SimpleStmt:
	EmptyStmt
	{
		$$ = $1 //TODO 242
	}
|	ExpressionStmt
	{
		$$ = $1 //TODO 243
	}
|	SendStmt
	{
		$$ = $1 //TODO 244
	}
|	IncDecStmt
	{
		$$ = $1 //TODO 245
	}
|	Assignment
	{
		$$ = $1 //TODO 246
	}
|	ShortVarDecl
	{
		$$ = $1 //TODO 247
	}

Slice:
	'[' Slice1
	{
		$$ = []Slice{"[", $2} //TODO 248
	}
|	Slice2 ']'
	{
		$$ = []Slice{$1, "]"} //TODO 249
	}

Slice1:
	Slice11 ':' Slice12
	{
		$$ = []Slice1{$1, ":", $3} //TODO 250
	}

Slice11:
	/* EMPTY */
	{
		$$ = nil //TODO 251
	}
|	Expression
	{
		$$ = $1 //TODO 252
	}

Slice12:
	/* EMPTY */
	{
		$$ = nil //TODO 253
	}
|	Expression
	{
		$$ = $1 //TODO 254
	}

Slice2:
	Slice21 ':' Expression ':' Expression
	{
		$$ = []Slice2{$1, ":", $3, ":", $5} //TODO 255
	}

Slice21:
	/* EMPTY */
	{
		$$ = nil //TODO 256
	}
|	Expression
	{
		$$ = $1 //TODO 257
	}

SliceType:
	'[' ']' ElementType
	{
		$$ = []SliceType{"[", "]", $3} //TODO 258
	}

SourceFile:
	PackageClause ';' SourceFile1 SourceFile2
	{
		$$ = []SourceFile{$1, ";", $3, $4} //TODO 259
	}

SourceFile1:
	/* EMPTY */
	{
		$$ = []SourceFile1(nil) //TODO 260
	}
|	SourceFile1 ImportDecl ';'
	{
		$$ = append($1.([]SourceFile1), $2, ";") //TODO 261
	}

SourceFile2:
	/* EMPTY */
	{
		$$ = []SourceFile2(nil) //TODO 262
	}
|	SourceFile2 TopLevelDecl ';'
	{
		$$ = append($1.([]SourceFile2), $2, ";") //TODO 263
	}

Start:
	SourceFile
	{
		_parserResult = $1 //TODO 264
	}

Statement:
	Declaration
	{
		$$ = $1 //TODO 265
	}
|	LabeledStmt
	{
		$$ = $1 //TODO 266
	}
|	SimpleStmt
	{
		$$ = $1 //TODO 267
	}
|	GoStmt
	{
		$$ = $1 //TODO 268
	}
|	ReturnStmt
	{
		$$ = $1 //TODO 269
	}
|	BreakStmt
	{
		$$ = $1 //TODO 270
	}
|	ContinueStmt
	{
		$$ = $1 //TODO 271
	}
|	GotoStmt
	{
		$$ = $1 //TODO 272
	}
|	FallthroughStmt
	{
		$$ = $1 //TODO 273
	}
|	Block
	{
		$$ = $1 //TODO 274
	}
|	IfStmt
	{
		$$ = $1 //TODO 275
	}
|	SwitchStmt
	{
		$$ = $1 //TODO 276
	}
|	SelectStmt
	{
		$$ = $1 //TODO 277
	}
|	ForStmt
	{
		$$ = $1 //TODO 278
	}
|	DeferStmt
	{
		$$ = $1 //TODO 279
	}

StatementList:
	StatementList1
	{
		$$ = $1 //TODO 280
	}

StatementList1:
	/* EMPTY */
	{
		$$ = []StatementList1(nil) //TODO 281
	}
|	StatementList1 Statement ';'
	{
		$$ = append($1.([]StatementList1), $2, ";") //TODO 282
	}

StructType:
	STRUCT '{' StructType1 '}'
	{
		$$ = []StructType{"struct", "{", $3, "}"} //TODO 283
	}

StructType1:
	/* EMPTY */
	{
		$$ = []StructType1(nil) //TODO 284
	}
|	StructType1 FieldDecl ';'
	{
		$$ = append($1.([]StructType1), $2, ";") //TODO 285
	}

SwitchStmt:
	ExprSwitchStmt
	{
		$$ = $1 //TODO 286
	}
|	TypeSwitchStmt
	{
		$$ = $1 //TODO 287
	}

Tag:
	STRING_LIT
	{
		$$ = $1 //TODO 288
	}

TopLevelDecl:
	Declaration
	{
		$$ = $1 //TODO 289
	}
|	FunctionDecl
	{
		$$ = $1 //TODO 290
	}
|	MethodDecl
	{
		$$ = $1 //TODO 291
	}

Type:
	TypeName
	{
		$$ = $1 //TODO 292
	}
|	TypeLit
	{
		$$ = $1 //TODO 293
	}
|	'(' Type ')'
	{
		$$ = []Type{"(", $2, ")"} //TODO 294
	}

TypeAssertion:
	'.' '(' Type ')'
	{
		$$ = []TypeAssertion{".", "(", $3, ")"} //TODO 295
	}

TypeCaseClause:
	TypeSwitchCase ':' StatementList
	{
		$$ = []TypeCaseClause{$1, ":", $3} //TODO 296
	}

TypeDecl:
	TYPE TypeDecl1
	{
		$$ = []TypeDecl{"type", $2} //TODO 297
	}

TypeDecl1:
	TypeSpec
	{
		$$ = $1 //TODO 298
	}
|	'(' TypeDecl11 ')'
	{
		$$ = []TypeDecl1{"(", $2, ")"} //TODO 299
	}

TypeDecl11:
	/* EMPTY */
	{
		$$ = []TypeDecl11(nil) //TODO 300
	}
|	TypeDecl11 TypeSpec ';'
	{
		$$ = append($1.([]TypeDecl11), $2, ";") //TODO 301
	}

TypeList:
	Type TypeList1
	{
		$$ = []TypeList{$1, $2} //TODO 302
	}

TypeList1:
	/* EMPTY */
	{
		$$ = []TypeList1(nil) //TODO 303
	}
|	TypeList1 ',' Type
	{
		$$ = append($1.([]TypeList1), ",", $3) //TODO 304
	}

TypeLit:
	ArrayType
	{
		$$ = $1 //TODO 305
	}
|	StructType
	{
		$$ = $1 //TODO 306
	}
|	PointerType
	{
		$$ = $1 //TODO 307
	}
|	FunctionType
	{
		$$ = $1 //TODO 308
	}
|	InterfaceType
	{
		$$ = $1 //TODO 309
	}
|	SliceType
	{
		$$ = $1 //TODO 310
	}
|	MapType
	{
		$$ = $1 //TODO 311
	}
|	ChannelType
	{
		$$ = $1 //TODO 312
	}

TypeName:
	IDENTIFIER
	{
		$$ = $1 //TODO 313
	}
|	QualifiedIdent
	{
		$$ = $1 //TODO 314
	}

TypeSpec:
	IDENTIFIER Type
	{
		$$ = []TypeSpec{$1, $2} //TODO 315
	}

TypeSwitchCase:
	CASE TypeList
	{
		$$ = []TypeSwitchCase{"case", $2} //TODO 316
	}
|	DEFAULT
	{
		$$ = "default" //TODO 317
	}

TypeSwitchGuard:
	TypeSwitchGuard1 PrimaryExpr '.' '(' TYPE ')'
	{
		$$ = []TypeSwitchGuard{$1, $2, ".", "(", "type", ")"} //TODO 318
	}

TypeSwitchGuard1:
	/* EMPTY */
	{
		$$ = nil //TODO 319
	}
|	IDENTIFIER TOK3 /* ":=" */
	{
		$$ = []TypeSwitchGuard1{$1, ":="} //TODO 320
	}

TypeSwitchStmt:
	SWITCH TypeSwitchStmt1 TypeSwitchGuard '{' TypeSwitchStmt2 '}'
	{
		$$ = []TypeSwitchStmt{"switch", $2, $3, "{", $5, "}"} //TODO 321
	}

TypeSwitchStmt1:
	/* EMPTY */
	{
		$$ = nil //TODO 322
	}
|	SimpleStmt ';'
	{
		$$ = []TypeSwitchStmt1{$1, ";"} //TODO 323
	}

TypeSwitchStmt2:
	/* EMPTY */
	{
		$$ = []TypeSwitchStmt2(nil) //TODO 324
	}
|	TypeSwitchStmt2 TypeCaseClause
	{
		$$ = append($1.([]TypeSwitchStmt2), $2) //TODO 325
	}

UnaryExpr:
	PrimaryExpr
	{
		$$ = $1 //TODO 326
	}
|	UNARY_OP UnaryExpr
	{
		$$ = []UnaryExpr{$1, $2} //TODO 327
	}

Value:
	Expression
	{
		$$ = $1 //TODO 328
	}
|	LiteralValue
	{
		$$ = $1 //TODO 329
	}

VarDecl:
	VAR VarDecl1
	{
		$$ = []VarDecl{"var", $2} //TODO 330
	}

VarDecl1:
	VarSpec
	{
		$$ = $1 //TODO 331
	}
|	'(' VarDecl11 ')'
	{
		$$ = []VarDecl1{"(", $2, ")"} //TODO 332
	}

VarDecl11:
	/* EMPTY */
	{
		$$ = []VarDecl11(nil) //TODO 333
	}
|	VarDecl11 VarSpec ';'
	{
		$$ = append($1.([]VarDecl11), $2, ";") //TODO 334
	}

VarSpec:
	IdentifierList VarSpec1
	{
		$$ = []VarSpec{$1, $2} //TODO 335
	}

VarSpec1:
	Type VarSpec11
	{
		$$ = []VarSpec1{$1, $2} //TODO 336
	}
|	'=' ExpressionList
	{
		$$ = []VarSpec1{"=", $2} //TODO 337
	}

VarSpec11:
	/* EMPTY */
	{
		$$ = nil //TODO 338
	}
|	'=' ExpressionList
	{
		$$ = []VarSpec11{"=", $2} //TODO 339
	}

%%

//TODO remove demo stuff below

var _parserResult interface{}

type (
	AnonymousField interface{}
	AnonymousField1 interface{}
	ArgumentList interface{}
	ArgumentList1 interface{}
	ArrayLength interface{}
	ArrayType interface{}
	Assignment interface{}
	BaseType interface{}
	BaseTypeName interface{}
	BasicLit interface{}
	Block interface{}
	BreakStmt interface{}
	BreakStmt1 interface{}
	BuiltinArgs interface{}
	BuiltinArgs1 interface{}
	BuiltinCall interface{}
	BuiltinCall1 interface{}
	BuiltinCall11 interface{}
	Call interface{}
	Call1 interface{}
	Call11 interface{}
	Channel interface{}
	ChannelType interface{}
	ChannelType1 interface{}
	CommCase interface{}
	CommCase1 interface{}
	CommClause interface{}
	CompositeLit interface{}
	Condition interface{}
	ConstDecl interface{}
	ConstDecl1 interface{}
	ConstDecl11 interface{}
	ConstSpec interface{}
	ConstSpec1 interface{}
	ConstSpec11 interface{}
	ContinueStmt interface{}
	ContinueStmt1 interface{}
	Conversion interface{}
	Conversion1 interface{}
	Declaration interface{}
	DeferStmt interface{}
	Element interface{}
	Element1 interface{}
	ElementIndex interface{}
	ElementList interface{}
	ElementList1 interface{}
	ElementType interface{}
	EmptyStmt interface{}
	ExprCaseClause interface{}
	ExprSwitchCase interface{}
	ExprSwitchStmt interface{}
	ExprSwitchStmt1 interface{}
	ExprSwitchStmt2 interface{}
	ExprSwitchStmt3 interface{}
	Expression interface{}
	ExpressionList interface{}
	ExpressionList1 interface{}
	ExpressionStmt interface{}
	FallthroughStmt interface{}
	FieldDecl interface{}
	FieldDecl1 interface{}
	FieldDecl2 interface{}
	FieldName interface{}
	ForClause interface{}
	ForClause1 interface{}
	ForClause2 interface{}
	ForClause3 interface{}
	ForStmt interface{}
	ForStmt1 interface{}
	ForStmt11 interface{}
	Function interface{}
	FunctionBody interface{}
	FunctionDecl interface{}
	FunctionDecl1 interface{}
	FunctionLit interface{}
	FunctionName interface{}
	FunctionType interface{}
	GoStmt interface{}
	GotoStmt interface{}
	IdentifierList interface{}
	IdentifierList1 interface{}
	IfStmt interface{}
	IfStmt1 interface{}
	IfStmt2 interface{}
	IfStmt21 interface{}
	ImportDecl interface{}
	ImportDecl1 interface{}
	ImportDecl11 interface{}
	ImportPath interface{}
	ImportSpec interface{}
	ImportSpec1 interface{}
	ImportSpec11 interface{}
	IncDecStmt interface{}
	IncDecStmt1 interface{}
	Index interface{}
	InitStmt interface{}
	InterfaceType interface{}
	InterfaceType1 interface{}
	InterfaceTypeName interface{}
	Key interface{}
	KeyType interface{}
	Label interface{}
	LabeledStmt interface{}
	Literal interface{}
	LiteralType interface{}
	LiteralValue interface{}
	LiteralValue1 interface{}
	LiteralValue11 interface{}
	MapType interface{}
	MethodDecl interface{}
	MethodDecl1 interface{}
	MethodExpr interface{}
	MethodName interface{}
	MethodSpec interface{}
	Operand interface{}
	OperandName interface{}
	PackageClause interface{}
	PackageName interface{}
	ParameterDecl interface{}
	ParameterDecl1 interface{}
	ParameterDecl2 interface{}
	ParameterList interface{}
	ParameterList1 interface{}
	Parameters interface{}
	Parameters1 interface{}
	Parameters11 interface{}
	PointerType interface{}
	PostStmt interface{}
	PrimaryExpr interface{}
	QualifiedIdent interface{}
	RangeClause interface{}
	RangeClause1 interface{}
	Receiver interface{}
	Receiver1 interface{}
	Receiver2 interface{}
	ReceiverType interface{}
	RecvExpr interface{}
	RecvStmt interface{}
	RecvStmt1 interface{}
	RecvStmt11 interface{}
	Result interface{}
	ReturnStmt interface{}
	ReturnStmt1 interface{}
	SelectStmt interface{}
	SelectStmt1 interface{}
	Selector interface{}
	SendStmt interface{}
	ShortVarDecl interface{}
	Signature interface{}
	Signature1 interface{}
	SimpleStmt interface{}
	Slice interface{}
	Slice1 interface{}
	Slice11 interface{}
	Slice12 interface{}
	Slice2 interface{}
	Slice21 interface{}
	SliceType interface{}
	SourceFile interface{}
	SourceFile1 interface{}
	SourceFile2 interface{}
	Start interface{}
	Statement interface{}
	StatementList interface{}
	StatementList1 interface{}
	StructType interface{}
	StructType1 interface{}
	SwitchStmt interface{}
	Tag interface{}
	TopLevelDecl interface{}
	Type interface{}
	TypeAssertion interface{}
	TypeCaseClause interface{}
	TypeDecl interface{}
	TypeDecl1 interface{}
	TypeDecl11 interface{}
	TypeList interface{}
	TypeList1 interface{}
	TypeLit interface{}
	TypeName interface{}
	TypeSpec interface{}
	TypeSwitchCase interface{}
	TypeSwitchGuard interface{}
	TypeSwitchGuard1 interface{}
	TypeSwitchStmt interface{}
	TypeSwitchStmt1 interface{}
	TypeSwitchStmt2 interface{}
	UnaryExpr interface{}
	Value interface{}
	VarDecl interface{}
	VarDecl1 interface{}
	VarDecl11 interface{}
	VarSpec interface{}
	VarSpec1 interface{}
	VarSpec11 interface{}
)
	
func _dump() {
	s := fmt.Sprintf("%#v", _parserResult)
	s = strings.Replace(s, "%", "%%", -1)
	s = strings.Replace(s, "{", "{%i\n", -1)
	s = strings.Replace(s, "}", "%u\n}", -1)
	s = strings.Replace(s, ", ", ",\n", -1)
	var buf bytes.Buffer
	strutil.IndentFormatter(&buf, ". ").Format(s)
	buf.WriteString("\n")
	a := strings.Split(buf.String(), "\n")
	for _, v := range a {
		if strings.HasSuffix(v, "(nil)") || strings.HasSuffix(v, "(nil),") {
			continue
		}
	
		fmt.Println(v)
	}
}

// End of demo stuff
