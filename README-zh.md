...
完整 API 文档请参阅 [Go 包文档](https://pkg.go.dev/github.com/next-bin/go-sqlite3)。
...
驱动通过 `github.com/next-bin/go-sqlite3/vtab` 包暴露纯 Go API，用于实现 SQLite 虚拟表模块。可以将任意数据源（如向量索引、CSV 文件、远程 API）作为 SQL 表，并与 SQLite 的查询优化器集成。
